Shader "MyShader/Grass/GrassPhone"
{
    Properties{
    ...
    SubShader{
        ...
        Pass
        {
            ...
            {   
                ...
                _Height = _Height + (random*random-0.5f)/5;
                ...
                float horizontalValue=0.6f;// 横向偏移
                float lowerValue=0.6f;//垂直压低
                ...
                for (int i = 0; i < vertexCount; i++)
                {
                    ...
                    float heightFactor = (v[i].pos.y - root.y) / _Height;// 计算草的高度比例
                    ...
                    // 添加无风状态自然弯曲
                    float naturalBend = curve  * random*0.25f;
                    ...
                    {    
                        ...
                        // 计算偏移和弯曲效果
                        playerBend = playerDir * playerInfluence*0.3f;
                        playerVerticalOffset = playerInfluence*0.5f;
                    }
                    ...
                    // 使用风场纹理的B通道来控制风向
                    float windAngle = windSample.b * UNITY_TWO_PI; // 将0-1的值映射到0-2π
                    float2 windDir = float2(cos(windAngle), sin(windAngle));

                    // 根据顶点高度决定风的影响强度
                    
                    // float bendFactor = saturate((heightFactor - bendStartRatio) / (1.0 - bendStartRatio));

                    float horizontalCurve = curve * horizontalValue;
                    float verticalCurve = curve * lowerValue;
                    ...
                    // 添加小幅高频摆动
                    float microMove = sin(_Time.y  + root.x  + root.z  + heightFactor) * 0.02f * heightFactor;
                    v[i].pos.xz += (float2(microMove, microMove) * windBaseStrength+float2(microMove, microMove)*1.5f+playerBend);
